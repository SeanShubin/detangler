...
class StringToStandaloneFunctionTest extends FunSuite {
  test("string to standalone") {
    ...
    assert(f("com/seanshubin/Hi$baz") === Some(Standalone(Seq("missing-level-1", "missing-level-2", "Hi"))))
    ...
    assert(f("com/seanshubin/Hi") === Some(Standalone(Seq("missing-level-1", "missing-level-2", "Hi"))))
  }
  ...
  test("paths at 4 levels deep") {
    ...
    assert(standaloneAtLevel(4, Seq("p", "q", "r")) === Standalone(Seq("p", "q", "missing-level-3", "r")))
    assert(standaloneAtLevel(4, Seq("s", "t")) === Standalone(Seq("s", "missing-level-2", "missing-level-3", "t")))
    assert(standaloneAtLevel(4, Seq("u")) === Standalone(Seq("missing-level-1", "missing-level-2", "missing-level-3", "u")))
    ...
    assert(standaloneAtLevel(3, Seq("s", "t")) === Standalone(Seq("s", "missing-level-2", "t")))
    assert(standaloneAtLevel(3, Seq("u")) === Standalone(Seq("missing-level-1", "missing-level-2", "u")))
    ...
    assert(standaloneAtLevel(2, Seq("u")) === Standalone(Seq("missing-level-1", "u")))
